...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wikimedia Commons');
}
...
async function cmGet(params: URLSearchParams): Promise<unknown> {
  const res = await pwFetch(`${BASE}?${params}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'Commons');
  return res.json();
}
