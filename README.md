...
`tools/list` at `https://gateway.pipeworx.io/commons-wikimedia/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/commons_wikimedia_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"Eiffel Tower"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/commons_wikimedia_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "commons-wikimedia": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-commons-wikimedia"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-commons-wikimedia
```
